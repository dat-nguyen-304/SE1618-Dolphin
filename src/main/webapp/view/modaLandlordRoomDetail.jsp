            ...
            <div class="p-6 space-y-6">
                <c:forEach items="${requestScope.roomList}" var="room">
                    ...
                    <c:if test="${room.status == 1}">
                        <form action="/sakura/landlord/room-detail" method="post" class="inline-block">
                            <button type="submit" name="roomId" value="${room.roomID}" class="px-[10px] py-[5px] mr-[10px] rounded bg-gray-200 bg-[#17535B] text-white hover:bg-[#13484F] duration-150">${room.roomNumber}</button>
                        </form>
                    </c:if>
                </c:forEach>
            </div>
    ...
    <div class="modal-container bg-white w-1/3 mx-auto rounded shadow-lg z-50 overflow-y-auto">
        ...
        <div class="modal-content">
            ...
            <div class="p-4">
                ...
                <p class="mb-5 text-lg text-center font-normal text-gray-800">Bạn có chắc chắn muốn xóa?</p>
            </div>

            <!--Footer-->
            <div class="flex justify-end p-[20px]">
                <button class="deleteRoommodal1-close px-5 text-[#7e7e7e] py-2 rounded hover:text-[#FF6532]">Huỷ</button>
                <button onclick="deleteRoom()" class="deleteRoom px-5 py-2 rounded bg-[#B51905] text-white hover:bg-[#951504] mr-2">Chắc chắn</button>
            </div>
        </div>
    </div>
</div>
